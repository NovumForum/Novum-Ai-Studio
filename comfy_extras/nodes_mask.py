...
class MaskComposite(IO.ComfyNode):
    ...
    def define_schema(cls):
        return IO.Schema(
            ...
            category="mask",
            description="Position and blend two masks together using logical or arithmetic blending operations.",
            inputs=[
                IO.Mask.Input("destination", tooltip="The background mask that serves as the base canvas."),
                IO.Mask.Input("source", tooltip="The foreground mask to place and blend onto the destination."),
                IO.Int.Input("x", default=0, min=0, max=nodes.MAX_RESOLUTION, step=1, tooltip="Horizontal offset (in pixels) for placing the source mask onto the destination."),
                IO.Int.Input("y", default=0, min=0, max=nodes.MAX_RESOLUTION, step=1, tooltip="Vertical offset (in pixels) for placing the source mask onto the destination."),
                IO.Combo.Input("operation", options=["multiply", "add", "subtract", "and", "or", "xor"], tooltip="The mathematical or logical operation used to combine the overlapping parts of the masks."),
            ],
            outputs=[IO.Mask.Output()],
        )
...
class FeatherMask(IO.ComfyNode):
    ...
    def define_schema(cls):
        return IO.Schema(
            ...
            category="mask",
            description="Soften the edges of a mask by creating a linear fade-out gradient inward from the borders.",
            inputs=[
                IO.Mask.Input("mask", tooltip="The input mask to soften/feather."),
                IO.Int.Input("left", default=0, min=0, max=nodes.MAX_RESOLUTION, step=1, tooltip="Amount of pixels to fade along the left edge."),
                IO.Int.Input("top", default=0, min=0, max=nodes.MAX_RESOLUTION, step=1, tooltip="Amount of pixels to fade along the top edge."),
                IO.Int.Input("right", default=0, min=0, max=nodes.MAX_RESOLUTION, step=1, tooltip="Amount of pixels to fade along the right edge."),
                IO.Int.Input("bottom", default=0, min=0, max=nodes.MAX_RESOLUTION, step=1, tooltip="Amount of pixels to fade along the bottom edge."),
            ],
            outputs=[IO.Mask.Output()],
        )
...
class GrowMask(IO.ComfyNode):
    ...
    def define_schema(cls):
        return IO.Schema(
            ...
            category="mask",
            description="Expand or shrink a mask by a specified number of pixels. Use positive values to grow, negative to shrink.",
            inputs=[
                IO.Mask.Input("mask", tooltip="The input mask to expand or shrink."),
                IO.Int.Input("expand", default=0, min=-nodes.MAX_RESOLUTION, max=nodes.MAX_RESOLUTION, step=1, tooltip="Number of pixels to grow (positive values) or shrink (negative values) the mask."),
                IO.Boolean.Input("tapered_corners", default=True, advanced=True, tooltip="When enabled, rounds/tapers the corners of the mask during expansion or erosion."),
            ],
            outputs=[IO.Mask.Output()],
        )
