...
class ImageColorToMask(IO.ComfyNode):
    ...
    def define_schema(cls):
        return IO.Schema(
            ...
            inputs=[
                IO.Image.Input("image"),
                IO.Int.Input("color", default=0, min=0, max=0xFFFFFF, step=1, display_mode=IO.NumberDisplay.color, tooltip="The color to convert to a mask. Matches pixels with this exact color."),
            ],
            outputs=[IO.Mask.Output()],
        )
