...
def ensure_path_within(base_dir, folder_name):
    base_dir = os.path.abspath(base_dir)
    full_path = os.path.abspath(os.path.join(base_dir, folder_name))
    if os.path.commonpath([base_dir, full_path]) != base_dir:
        raise ValueError(
            f"Access denied: {folder_name} is outside of permitted directory"
        )
    return full_path
...
class LoadImageDataSetFromFolderNode(io.ComfyNode):
    ...
    def execute(cls, folder):
        sub_input_dir = ensure_path_within(folder_paths.get_input_directory(), folder)
...
class SaveImageDataSetToFolderNode(io.ComfyNode):
    ...
    def execute(cls, images, folder_name, filename_prefix):
        ...
        output_dir = ensure_path_within(folder_paths.get_output_directory(), folder_name)
        # Ensure filename_prefix doesn't cause traversal when joined with output_dir
        ensure_path_within(output_dir, filename_prefix)
...
class LoadTrainingDataset(io.ComfyNode):
    ...
    def execute(cls, folder_name):
        ...
        dataset_dir = ensure_path_within(
            folder_paths.get_output_directory(), folder_name
        )
