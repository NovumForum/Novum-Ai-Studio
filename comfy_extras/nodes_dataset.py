...
def safe_join(base, path):
    ...
    try:
        common = os.path.commonpath([os.path.abspath(base), joined])
    except ValueError:
        # Handles cases where base and joined are on different drives on Windows
        raise ValueError(f"Path traversal detected: {path} is not in {base}")
