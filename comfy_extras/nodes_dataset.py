...
def safe_join(base, path):
    ...
    joined_path = os.path.abspath(os.path.join(base, path))

    try:
        if os.path.commonpath([base, joined_path]) != base:
            raise ValueError(
                f"Path traversal detected: {joined_path} is outside of {base}"
            )
    except ValueError:
        raise ValueError(f"Path traversal detected: {joined_path} is outside of {base}")
