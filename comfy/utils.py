...
def safe_join(base_path, *paths, create_dir=False):
    """Safely join paths ensuring the result is inside base_path. Prevents path traversal."""
    import os
    base_path = os.path.abspath(base_path)
    final_path = os.path.abspath(os.path.join(base_path, *paths))

    if os.path.commonpath([base_path, final_path]) != base_path:
        raise ValueError(f"Path traversal detected: {final_path} is outside base path {base_path}")
