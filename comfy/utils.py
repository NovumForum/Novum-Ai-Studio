...
def safe_join(base, *paths, create_dir=False):
    ...
    Ensures the resulting path is within the base directory to prevent path traversal.
    """
    base = os.path.abspath(base)
    path = os.path.abspath(os.path.join(base, *paths))
    if os.path.commonpath([base, path]) != base:
        raise ValueError(
            "Path traversal detected! The resulting path is outside the base directory."
        )
    if create_dir:
