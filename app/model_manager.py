...
class ModelFileManager:
    ...
    def add_routes(self, routes):
        ...
        async def get_model_preview(request):
            ...
            raw_path_index = request.match_info.get("path_index", None)
            ...
            if path_index < 0 or path_index >= len(folders[0]):
                return web.Response(status=400, text="path_index out of bounds")

            folder = os.path.abspath(folders[0][path_index])
            full_filename = os.path.abspath(os.path.join(folder, filename))

            # Security check: Prevent path traversal outside designated model folder
