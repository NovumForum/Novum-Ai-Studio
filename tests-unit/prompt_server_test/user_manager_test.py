...
async def test_delete_userdata_file(aiohttp_client, app, tmp_path):
    # Create initial file
    with open(tmp_path / "delete_target.txt", "w") as f:
        f.write("test content")

    client = await aiohttp_client(app)
    resp = await client.delete("/userdata/delete_target.txt")
    assert resp.status == 204
    assert not os.path.exists(tmp_path / "delete_target.txt")


async def test_delete_userdata_directory_returns_error(aiohttp_client, app, tmp_path):
    # Create a directory
    os.makedirs(tmp_path / "target_dir")

    client = await aiohttp_client(app)
    resp = await client.delete("/userdata/target_dir")
    assert resp.status == 400
    assert await resp.text() == "Requested path is a directory"
    assert os.path.exists(tmp_path / "target_dir")
