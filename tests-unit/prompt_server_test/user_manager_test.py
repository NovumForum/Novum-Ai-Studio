...
async def test_delete_userdata_file(aiohttp_client, app, tmp_path):
    (tmp_path / "test.txt").write_text("content to delete")
    client = await aiohttp_client(app)

    resp = await client.delete("/userdata/test.txt")
    assert resp.status == 204
    assert not os.path.exists(tmp_path / "test.txt")


async def test_delete_userdata_directory_rejected(aiohttp_client, app, tmp_path):
    os.makedirs(tmp_path / "test_dir")
    client = await aiohttp_client(app)
    ...
    assert os.path.exists(tmp_path / "test_dir")
